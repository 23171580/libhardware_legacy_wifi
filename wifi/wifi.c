...
#elif defined SSV6051_WIFI_USED
    /* ssv6051 sdio wifi */
    #ifndef WIFI_DRIVER_MODULE_PATH
    #define WIFI_DRIVER_MODULE_PATH         "/system/vendor/modules/ssv6051.ko"
    #endif
    #ifndef WIFI_DRIVER_MODULE_NAME
    #define WIFI_DRIVER_MODULE_NAME         "ssv6051"
    #endif
    #ifndef WIFI_DRIVER_MODULE_ARG
    #define WIFI_DRIVER_MODULE_ARG 	    "stacfgpath=\"/etc/firmware/ssv6051-wifi.cfg\""
    #endif    
...
#if defined(RTL_WIFI_VENDOR) || defined(ESP_WIFI_VENDOR) || defined(SSV6051_WIFI_VENDOR)
...
int ensure_config_file_exists(const char *config_file)
{
    ...
    if (srcfd < 0) {
        ALOGE("Cannot open \"%s\": %s", P2P_CONFIG_TEMPLATE, strerror(errno));
        return -1;
    }
    ...
    while ((nread = TEMP_FAILURE_RETRY(read(srcfd, buf, sizeof(buf)))) != 0) {
        if (nread < 0) {
            ALOGE("Error reading \"%s\": %s", P2P_CONFIG_TEMPLATE, strerror(errno));
            close(srcfd);
            close(destfd);
            unlink(config_file);
            return -1;
...
int wifi_connect_to_supplicant()
{
    ...
    //snprintf(path, sizeof(path), "/data/misc/wifi/wpa_supplicant/%s", primary_iface);
...
#endif	
#if defined(SSV6051_WIFI_VENDOR)
    if (strncmp(cmd, "IFNAME=wlan0 DRIVER SETBAND", 27) == 0) {
        ALOGE("ssv wifi filter: cmd = %s",cmd);
        reply[*reply_len] = '\0';
        return 0;
    }
#endif
